function y = rp_RocAnalysis (Directory)
...
MinGap = [8 12 14 16 18]';

MinGap = [20 30 40 50 60 70 80 90 100 110 120 130 140]';

MinGap = [8 12 14 16 18 20 30 35 40 50]';

colors = 'kbgrykbgrykbgry';
allDetRateAt20Percent = [];

figure1 = figure;
axes1 = axes('Parent',figure1,'YGrid','on','XGrid','on','FontSize',24);
xlabel('False Positive Rate','FontSize',24); ylabel('True Detection Rate','FontSize',24);
...
for ii=1:size(MinGap)
    Directory = ['Results_mingap_' num2str(MinGap(ii)) filesep];
...
%               'DisplayName',sprintf('MinGap %d', MinGap(ii)), ...
%               'Color',colors(ii), ...
%               'Parent',axes1);

    %% GET detection rates at 20%
    meanDetRateAt20Percent = mean(DetectionRates(:,21));
    stdDetRateAt20percent = std(DetectionRates(:,21));
    allDetRateAt20Percent = [allDetRateAt20Percent; MinGap(ii) meanDetRateAt20Percent stdDetRateAt20percent];
...
figure;
errorbar(allDetRateAt20Percent(:,1),allDetRateAt20Percent(:,2), allDetRateAt20Percent(:,3));
